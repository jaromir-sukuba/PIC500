/*********************************************
* Modified by Jaromir Sukuba http://jaromir.xf.cz/, Date: 05 June 2013
* for XC8 compiler and PIC18F1825 device. 
*
*
...
#define EEPROM_MAJOR            2
#define EEPROM_MINOR            1
#define EEPROM_MAGIC            0
unsigned char CONFIG_PARAM_SW_MAJOR;
unsigned char CONFIG_PARAM_SW_MINOR;
const char terminal_init[30] = {"\x1B[0m\x1B[2J\x1B[0;0f"};
...
static unsigned int saddress=0;
...
void programcmd(unsigned char seqnum)
{
        ...
        switch(msg_buf[0]){
                ...
                while(i<msg_buf[4]){//synchLoops
                        ...
                        dly_ms(msg_buf[3]); //cmdexeDelay
                        i++;
                        spi_mastertransmit(msg_buf[8]);//cmd1
                        dly_ms(msg_buf[5]); //byteDelay
...
void terminalmode(unsigned char chr_nl)
{
	...
	unsigned char i;
	// Init terminal
	uart_sendstr(terminal_init);
	//
	uart_sendstr("AvrUsb500-1.7 terminal mode\r\n");
	uart_sendstr("Enter SW Version Major in hex [");
	utoa((char *)msg_buf, CONFIG_PARAM_SW_MAJOR, 16);
	...
	uart_sendstr("Enter SW Version Minor in hex [");
	utoa((char *)msg_buf, CONFIG_PARAM_SW_MINOR, 16);
	uart_sendstr((char *)msg_buf);
	uart_sendstr("]: ");
	i=terminalmode_readnum(chr_nl);
	if (i!=0xff){
	...
	uart_sendstr((char *)msg_buf);
	uart_sendstr(" (hex)\r\n");
	uart_sendstr("Ready. Just close the terminal. No reset needed.\r\n");
...
int main(void)
{
	...
	CONFIG_PARAM_SW_MAJOR=D_CONFIG_PARAM_SW_MAJOR;
        if (eeprom_read_byte(EEPROM_MAGIC) == 20){
